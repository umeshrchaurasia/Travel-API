...
class BatchPaymentController {
    ...
    async getProposalDetailsByAgent_Practo(req, res) {
        try {
            const { agentId, paymentStatus } = req.body;
            ...
            // Call stored procedure
            const [rows] = await db.query(
                'CALL GetProposalDetailsByAgent_Practo(?, ?)',
                [agentId, paymentStatus || null]
            );
            ...
            logger.error('GetProposalDetailsByAgent_Practo error:', error);
            base.send_response(
                "Error retrieving proposal details",
                null,
                res,
    ...
    // Insert batch payment Agent
    async insertBatchPayment_Practo(req, res) {
        try {
            const { agentCode, policyNo, totalAmount, paymentMode, utr, proposal_id } = req.body;
            ...
            const Practo_proposal_id = proposal_id.split('||').map(pn => pn.trim()).filter(Boolean);
            ...
            // Call the stored procedure
            const [rows] = await db.query(
                'CALL InsertBatchPayment_Agent_Practo(?, ?, ?, ?, ?, ?)',
                [agentCode, policyNo, totalAmount, paymentMode, utr || '', proposal_id]
            );

            // Get first row of result
            const result = rows[0][0];
            if (result && result.Status === 'Success') {
                logger.info(`Batch payment processed successfully: ${result.PaymentRef}`);

                base.send_response(
                    "Payment processed successfully",
                    {
                        paymentRefNo: result.PaymentRef, // Corrected to use PaymentRef
                        AgentId: result.AgentId,        // Corrected to use AgentId
                        message: result.message,
                        status: result.SavedStatus,
                        processedPolicies: policyNumbers.length,
                        processedPracto_proposal_id: Practo_proposal_id.length
                    },
                    res
                );
            } else {
     ...
     async getBatchPaymentsByStatus_Practo(req, res) {
        try {
            const { status } = req.body;
            ...
            // Call stored procedure
            const [rows] = await db.query(
                'CALL GetBatchPaymentsByAdminApproval_Practo(?)',
                [status]
            );
    ...
    async updateBatchPayment_Practo(req, res) {
        try {
            const { paymentRefNo, newStatus, utr, AgentId } = req.body;
            ...
            try {
                // Call stored procedure
                const [results] = await db.query(
                    'CALL UpdateBatchPayment_SP_Practo(?, ?, ?, ?)',
                    [paymentRefNo, newStatus, utr || null, AgentId]
                );
    ...
    async getProposalDetailsByAgent_AyushPay(req, res) {
        try {
            const { agentId, paymentStatus } = req.body;
            ...
            // Call stored procedure
            const [rows] = await db.query(
                'CALL GetProposalDetailsByAgent_AyushPay(?, ?)',
                [agentId, paymentStatus || null]
            );
            ...
            logger.error('GetProposalDetailsByAgent_ayush error:', error);
            base.send_response(
                "Error retrieving proposal details",
                null,
                res,
    ...
    // Insert batch payment Agent
    async insertBatchPayment_AyushPay(req, res) {
        try {
            const { agentCode, policyNo, totalAmount, paymentMode, utr, proposal_id } = req.body;
            ...
            // Call the stored procedure
            const [rows] = await db.query(
                'CALL InsertBatchPayment_Agent_AyushPay(?, ?, ?, ?, ?, ?)',
                [agentCode, policyNo, totalAmount, paymentMode, utr || '', proposal_id]
            );
            ...
            if (result && result.Status === 'Success') {
                ...
                base.send_response(
                    "Payment processed successfully",
                    {
                        ...
                        message: result.message,
                        status: result.SavedStatus,
                        processedPolicies: policyNo.length,
                        processedPracto_proposal_id: proposal_id.length
                    },
                    res
                );
            } else {
     ...
     async getBatchPaymentsByStatus_AyushPay(req, res) {
        try {
            const { status } = req.body;
            ...
            // Call stored procedure
            const [rows] = await db.query(
                'CALL GetBatchPaymentsByAdminApproval_AyushPay(?)',
                [status]
            );
    ...
    async updateBatchPayment_AyushPay(req, res) {
        try {
            const { paymentRefNo, newStatus, utr, AgentId } = req.body;
            ...
            try {
                // Call stored procedure
                const [results] = await db.query(
                    'CALL UpdateBatchPayment_SP_AyushPay(?, ?, ?, ?)',
                    [paymentRefNo, newStatus, utr || null, AgentId]
                );
