...
class PractoController {
        ...
        try {
            ...
            if (practoStatus === "success" && practoMessage === "Subscription created successfully") {
                ...
                const [insertResult] = await db.query(
                    ...
                    [AgentId, FullName, EmailID, MobileNumber, plan_id, Practo_PlanName]
                );
    ...
    async getPractoPlan(req, res) {
        try {
            // Call the stored procedure to get pending wallet applications
            const [rows] = await db.query('CALL get_practo_plan_list()');

            if (rows && rows[0] && rows[0].length > 0) {
                return base.send_response(" retrieved successfully", rows[0], res);
            } else {
                return base.send_response("No  found", [], res);
            }
        } catch (error) {
            logger.error('Error in Applications:', error);
