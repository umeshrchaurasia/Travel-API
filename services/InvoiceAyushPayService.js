...
class InvoiceAyushPayService {
    constructor() {
        this.pdfStorePath = './public/invoicesAyushPay/';
    ...
    ensureDirectories() {
        try {
            ...
            const sourceTemplate = path.join(__dirname, '../views/tax-invoice-inter_ayushpay.ejs');
            const destTemplate = path.join(this.templatesPath, 'tax-invoice-inter_ayushpay.ejs');
            ...
            const sourceUpfrontTemplate = path.join(__dirname, '../views/tax-invoice-inter_upfront_ayushpay.ejs');
            const destUpfrontTemplate = path.join(this.templatesPath, 'tax-invoice-inter_upfront_ayushpay.ejs');
    ...
    async generateInvoiceAyushPay(custId, invoiceData, policyLogoBase64, signatureBase64, callback) {
        ...
        try {
            ...
            const pdfUrl = `/invoicesAyushPay/Invoice${custId}.pdf`;
            ...
            const templateFileName = processed.Paymentmode === 'Upfront Commission'
                ? 'tax-invoice-inter_upfront_ayushpay.ejs'
                : 'tax-invoice-inter_ayushpay.ejs';
    ...
    generateHtmlOnly(custId, invoiceData, policyLogoBase64, signatureBase64, callback) {
        ...
        try {
            ...
            let templateFileName = 'tax-invoice-inter_ayushpay.ejs'; // Default template
            if (processed.Paymentmode === 'Upfront Commission') {
                templateFileName = 'tax-invoice-inter_upfront_ayushpay.ejs';
            }
    ...
    prepareInvoiceData(invoiceData) {
        ...
        subTotalAmount_Upfront = Math.round(parseFloat(invoiceData.Actual_PremiumAmount || 0));

        // Get the base amount (Rate) to calculate tax

        const payoutPercentage = parseFloat(invoiceData.Payout || 0);
        const state = (invoiceData.statecode || 'GUJARAT').toUpperCase();
        // const state = ('GUJARAT').toUpperCase();
        ...
        if (invoiceData.Paymentmode === 'Upfront Commission') {
            if (state === "GJ" || state === "GUJARAT") {
                numCgst = Math.round(invoiceData.GST * 0.5);
                numSgst = Math.round(invoiceData.GST * 0.5);
                ...
                numIgst = Math.round(invoiceData.GST);
                CGST = "-";
                SGST = "-";
        ...
        if (invoiceData.Paymentmode === 'Upfront Commission') {
            ...
            lessCommission = Math.round(invoiceData.Commissionamount);  // Math.round((subTotalAmount_Upfront * payoutPercentage) / 100);
            tdsAmount = Math.round(invoiceData.TDS); // 2% TDS on the commission amount

            lessCommissionWords_Upfront = this.numberToWords(lessCommission);

            // Calculate the new total amount
            let newTotal = invoiceData.totalamount;//(subTotalAmount_Upfront + numCgst + numSgst + numIgst + tdsAmount) - lessCommission;
            finalTotalAmount_Upfront = Math.round(Actual_PremiumAmount_Upfront) + Math.round(invoiceData.GST);
            finalWordsAmount_Upfront = this.numberToWords(finalTotalAmount_Upfront);

            finalpayout_commisionAmount = newTotal;


        } else if (invoiceData.Paymentmode === 'Discount') {
            ...
            Actual_PremiumAmount_Upfront=  Math.round(newTotal) - Math.round(invoiceData.GST);
            subTotalAmount_Upfront = Actual_PremiumAmount_Upfront;
...
module.exports = new InvoiceAyushPayService();
